...
print FFFF "$htmlfile";
	...
	print color("yellow") , "\n   [!] Started At : " . ctime() . "\n" . color("reset");
	print color("green") , "   [+] Script-Type : Wordpress\n" . color("reset");
...
while(<w>){
	...
	my $wpua = LWP::UserAgent->new;
    $wpua->agent("Mozilla/8.0");	# Pretend to be Mozilla

    my $wpreq = HTTP::Request->new(GET => "$url$fl");
    my $wpres = $wpua->request($wpreq);

if ($wpres->is_success or "$wpsource" =~ "$fl"){
